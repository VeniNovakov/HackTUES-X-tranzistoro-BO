...
<Card class="self-center">
  <form class="flex flex-col space-y-6" >
    ...
    <Label class="space-y-2">
      <span>Email</span>
      <Input type="text" name="email" placeholder="name@company.com" on:input={handleChangeCredential} bind:value={loginInfo.email} required />
    </Label>
    <Label class="space-y-2">
      <span>Password</span>
      <Input type="password" name="password" placeholder="•••••••••••••••" on:input={handleChangeCredential} required bind:value={loginInfo.password} />
    </Label>
    <Button on:click={handleSubmit} color="blue" class="w-full">Login</Button>
  </form>
  {#if showToast}
	<div transition:fade>
    <Toast class="absolute top-0 toast" on:close={() => showToast=false}>
      <CloseOutline slot="icon" class=" w-5 h-5 text-primary-500 bg-primary-100 dark:bg-primary-800 dark:text-primary-200" />
      Wrong credentials
    </Toast>
		</div>
  {/if}
