<script lang="ts">
	import { Input, Fileupload, Label, Listgroup, ListgroupItem, Dropdown, Checkbox, Radio, DropdownItem, Button } from "flowbite-svelte";
  ...
  let files = [] // FileList type
	...
	const handleSelection = (selection: InfoSelection, obj: string) =>{
		if(selection.type === "company" || selection.type === "name" || selection.type === "description"){
    ...
    axiosWithRetry.post(`${BASEURL}/facilities`, formData, {
        headers: {
		...
		.then(resp => {
			if(resp.status === 201){
				panelView.update(() =>{return CONTROL_PANEL_OPTIONS.MAIN})
			}
		}
...
<div class="flex-row flex items-center justify-between w-[90%] h-[90%] mx-[10%] m-auto">
	<div class="h-[90dvh] w-full object-scale-down z-10 ">
		<Map></Map>
	</div>
	<form class="ml-6 w-full">
		...
		<Dropdown class="overflow-scroll max-h-24 max-w-32 relative">
			{#each tags as tag}
			<Checkbox on:click={() =>
		...
		<Dropdown class="overflow-scroll max-h-24 z-50">
			{#each departments as department}
			<Checkbox on:click={() => handleSelection({type: "departments"}, department)} checked={selectedInfo.departments.includes(department)}>{department}</Checkbox>
			{/each}
		</Dropdown>
		</div>
